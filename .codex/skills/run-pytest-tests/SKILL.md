...
description: Run pytest tests in the niflheim repository. Use when asked to run Python tests, pytest, test files, test directories, or the full pytest suite. Use system Python without a virtualenv. Prefer pytest-xdist with -n auto --dist loadfile because this repo's integration tests are split per file for loadfile scheduling.
---
