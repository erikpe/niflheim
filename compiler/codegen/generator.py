...
class CodeGenerator:
    ...
    def emit_root_frame_setup(self, layout: FunctionLayout, *, root_count: int, first_root_offset: int) -> None:
        ...
        self.asm.instr(f"mov rcx, {self.thread_state_roots_top_operand('rax')}")
        self.asm.instr(f"mov {self.root_frame_prev_operand('rdi')}, rcx")
        self.asm.instr(f"mov {self.root_frame_slot_count_operand('rdi')}, {root_count}")
        self.asm.instr(f"mov {self.root_frame_reserved_operand('rdi')}, 0")
        self.asm.instr(f"lea rcx, [rbp - {abs(first_root_offset)}]")
