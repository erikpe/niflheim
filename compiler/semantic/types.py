...
def semantic_type_ref_from_type_info(current_module_path: ModulePath, type_info: TypeInfo) -> SemanticTypeRef:
    if type_info.kind == "callable":
        ...
        param_types = tuple(semantic_type_ref_from_type_info(current_module_path, param) for param in type_info.callable_params)
    ...
    if type_info.kind == "interface":
        interface_id = _interface_id_from_type_name(current_module_path, type_info.name)
        canonical_name = type_info.name if interface_id is None else _qualified_nominal_name(interface_id.module_path, interface_id.name)
        return SemanticTypeRef(
            kind="interface",
    ...
    if type_info.kind == "reference":
        class_id = _class_id_from_type_name(current_module_path, type_info.name)
        canonical_name = type_info.name if class_id is None else _qualified_nominal_name(class_id.module_path, class_id.name)
        return SemanticTypeRef(
            kind="reference",
            canonical_name=canonical_name,
            display_name=type_info.name,
...
def best_effort_semantic_type_ref_from_name(
    ...
    type_name: str,
    *,
    nominal_kind: Literal["reference", "interface"] = "reference",
) -> SemanticTypeRef:
    text = type_name.strip()
    if is_function_type_name(text):
        params_text, return_text = _split_function_type(text)
        param_types = tuple(
            best_effort_semantic_type_ref_from_name(current_module_path, param_text)
            for param_text in _split_top_level(params_text)
            if param_text
        )
        return_type = best_effort_semantic_type_ref_from_name(current_module_path, return_text)
        return SemanticTypeRef(
            kind="callable",
            canonical_name=f"fn({', '.join(semantic_type_canonical_name(param) for param in param_types)}) -> {semantic_type_canonical_name(return_type)}",
            display_name=f"fn({', '.join(semantic_type_display_name(param) for param in param_types)}) -> {semantic_type_display_name(return_type)}",
    ...
    if is_array_type_name(text):
        element_type = best_effort_semantic_type_ref_from_name(current_module_path, text[:-2], nominal_kind=nominal_kind)
        return SemanticTypeRef(
            kind="reference",
            canonical_name=f"{semantic_type_canonical_name(element_type)}[]",
            display_name=f"{semantic_type_display_name(element_type)}[]",
    ...
    if nominal_kind == "interface":
        interface_id = _interface_id_from_type_name(current_module_path, text)
        canonical_name = text if interface_id is None else _qualified_nominal_name(interface_id.module_path, interface_id.name)
        return SemanticTypeRef(
            kind="interface",
            canonical_name=canonical_name,
            display_name=text,
    ...
    class_id = _class_id_from_type_name(current_module_path, text)
...
def semantic_type_ref_for_interface_id(interface_id: InterfaceId, *, display_name: str | None = None) -> SemanticTypeRef:
    return SemanticTypeRef(
        kind="interface",
...
def _class_id_from_type_name(current_module_path: ModulePath, type_name: str) -> ClassId | None:
...
def _interface_id_from_type_name(current_module_path: ModulePath, type_name: str) -> InterfaceId | None:
    if not _is_user_nominal_type_name(type_name):
        return None
    if "::" in type_name:
