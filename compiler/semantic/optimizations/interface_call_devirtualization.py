...
from .helpers.closed_world_dispatch import (
    build_closed_world_dispatch_index,
    resolve_closed_world_interface_method,
    resolve_closed_world_virtual_method,
)
...
class _DevirtualizationStats:
    ...
    closed_world_interface_calls: int = 0
    closed_world_virtual_calls: int = 0
    closed_world_interface_dispatches: int = 0
    closed_world_virtual_dispatches: int = 0
...
def interface_call_devirtualization(program: SemanticProgram) -> SemanticProgram:
    ...
    closed_world_index = build_closed_world_dispatch_index(program, compatibility_index, dispatch_index)
    stats = _DevirtualizationStats()
    optimized_program = rewrite_program_structure(
        program,
        rewrite_field=lambda field: _rewrite_field(field, compatibility_index, dispatch_index, closed_world_index, stats),
        rewrite_function=lambda fn: _rewrite_function(fn, compatibility_index, dispatch_index, closed_world_index, stats),
        rewrite_method=lambda method: _rewrite_method(method, compatibility_index, dispatch_index, closed_world_index, stats),
    )
    logger.debugv(
        1,
        "Optimization pass interface_call_devirtualization devirtualized %d interface calls, devirtualized %d virtual calls, specialized %d structural interface dispatches, specialized %d structural virtual dispatches, recovered %d array runtime dispatches, closed-world rewrote %d interface calls, %d virtual calls, %d structural interface dispatches, %d structural virtual dispatches, skipped %d non-local receivers, skipped %d cases without exact receiver type",
        ...
        stats.closed_world_interface_calls,
        stats.closed_world_virtual_calls,
        stats.closed_world_interface_dispatches,
        stats.closed_world_virtual_dispatches,
...
def _rewrite_field(
    ...
    return replace(
        field,
        initializer=_rewrite_expr(field.initializer, NarrowState.empty(), compatibility_index, dispatch_index, closed_world_index, stats),
    )
...
def _rewrite_nested_block(
    ...
    for stmt in block.statements:
        rewritten_stmt, current_state = _rewrite_stmt(
            stmt,
            current_state,
            compatibility_index,
            dispatch_index,
...
def _rewrite_stmt(
    ...
    if isinstance(stmt, SemanticBlock):
        return _rewrite_nested_block(stmt, state, compatibility_index, dispatch_index, closed_world_index, stats)

    if isinstance(stmt, SemanticVarDecl):
        initializer = None
        if stmt.initializer is not None:
            initializer = _rewrite_expr(
                stmt.initializer,
                state,
                compatibility_index,
                dispatch_index,
    ...
    if isinstance(stmt, SemanticAssign):
        target = _rewrite_lvalue(stmt.target, state, compatibility_index, dispatch_index, closed_world_index, stats)
        value = _rewrite_expr(stmt.value, state, compatibility_index, dispatch_index, closed_world_index, stats)
        next_state = state.fork()
    ...
    if isinstance(stmt, SemanticExprStmt):
        return (
            replace(
                stmt,
                expr=_rewrite_expr(stmt.expr, state, compatibility_index, dispatch_index, closed_world_index, stats),
            ),
            state,
        )
    ...
    if isinstance(stmt, SemanticReturn):
        value = (
            ...
            else _rewrite_expr(stmt.value, state, compatibility_index, dispatch_index, closed_world_index, stats)
        )
        return replace(stmt, value=value), state

    if isinstance(stmt, SemanticIf):
        rewritten_condition = _rewrite_expr(
            stmt.condition,
            state,
            compatibility_index,
            dispatch_index,
    ...
    if isinstance(stmt, SemanticForIn):
        rewritten_collection = _rewrite_expr(
            stmt.collection,
            state,
            compatibility_index,
            dispatch_index,
...
def _rewrite_lvalue(
    ...
    if isinstance(target, IndexLValue):
        rewritten_target = _rewrite_expr(
            target.target,
            state,
            compatibility_index,
            dispatch_index,
        ...
        return replace(
            ...
            index=_rewrite_expr(target.index, state, compatibility_index, dispatch_index, closed_world_index, stats),
    ...
    if isinstance(target, SliceLValue):
        ...
        return replace(
            target,
            target=rewritten_target,
            begin=_rewrite_expr(target.begin, state, compatibility_index, dispatch_index, closed_world_index, stats),
            end=_rewrite_expr(target.end, state, compatibility_index, dispatch_index, closed_world_index, stats),
            dispatch=_maybe_specialize_structural_dispatch(
                target.dispatch,
                rewritten_target,
...
def _rewrite_expr(
    ...
    if isinstance(expr, MethodRefExpr):
        receiver = (
            ...
            else _rewrite_expr(expr.receiver, state, compatibility_index, dispatch_index, closed_world_index, stats)
        )
    ...
    if isinstance(expr, UnaryExprS):
        return replace(
            expr,
            operand=_rewrite_expr(expr.operand, state, compatibility_index, dispatch_index, closed_world_index, stats),
        )

    if isinstance(expr, BinaryExprS):
        return replace(
            expr,
            left=_rewrite_expr(expr.left, state, compatibility_index, dispatch_index, closed_world_index, stats),
            right=_rewrite_expr(expr.right, state, compatibility_index, dispatch_index, closed_world_index, stats),
        )
    ...
    if isinstance(expr, CallExprS):
        rewritten_args = [
            _rewrite_expr(arg, state, compatibility_index, dispatch_index, closed_world_index, stats)
    ...
    if isinstance(expr, ArrayLenExpr):
        return replace(
            expr,
            target=_rewrite_expr(expr.target, state, compatibility_index, dispatch_index, closed_world_index, stats),
        )
    ...
    if isinstance(expr, IndexReadExpr):
        ...
        return replace(
            expr,
            target=rewritten_target,
            index=_rewrite_expr(expr.index, state, compatibility_index, dispatch_index, closed_world_index, stats),
            dispatch=_maybe_specialize_structural_dispatch(
                expr.dispatch,
                rewritten_target,
    ...
    if isinstance(expr, SliceReadExpr):
        ...
        return replace(
            expr,
            target=rewritten_target,
            begin=_rewrite_expr(expr.begin, state, compatibility_index, dispatch_index, closed_world_index, stats),
            end=_rewrite_expr(expr.end, state, compatibility_index, dispatch_index, closed_world_index, stats),
            dispatch=_maybe_specialize_structural_dispatch(
                expr.dispatch,
                rewritten_target,
...
def _maybe_devirtualize_call_target(
    ...
    if exact_type is not None and exact_type.class_id is not None:
    ...
    closed_world_method_id = resolve_closed_world_interface_method(closed_world_index, target.method_id)
    if closed_world_method_id is None:
        return target

    stats.closed_world_interface_calls += 1
    return InstanceMethodCallTarget(method_id=closed_world_method_id, access=target.access)
...
def _maybe_specialize_structural_dispatch(
    ...
    if not isinstance(dispatch, InterfaceDispatch):
        ...
        closed_world_method_id = resolve_closed_world_virtual_method(
            closed_world_index,
            dispatch.receiver_class_id,
            dispatch.slot_owner_class_id,
            dispatch.method_name,
        )
        if closed_world_method_id is None:
            return dispatch

        stats.closed_world_virtual_dispatches += 1
        return MethodDispatch(method_id=closed_world_method_id)
    ...
    closed_world_method_id = resolve_closed_world_interface_method(closed_world_index, dispatch.method_id)
    if closed_world_method_id is None:
        return dispatch

    stats.closed_world_interface_dispatches += 1
    return MethodDispatch(method_id=closed_world_method_id)
...
def _maybe_devirtualize_virtual_call_target(
    ...
    closed_world_method_id = resolve_closed_world_virtual_method(
        closed_world_index,
        receiver_class_id,
        target.slot_owner_class_id,
        target.slot_method_name,
    )
    if closed_world_method_id is None:
        return target

    stats.closed_world_virtual_calls += 1
    return InstanceMethodCallTarget(method_id=closed_world_method_id, access=target.access)
