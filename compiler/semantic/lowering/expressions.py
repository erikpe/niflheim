...
def lower_expr(typecheck_ctx: TypeCheckContext, symbol_index: ProgramSymbolIndex, expr: Expression) -> SemanticExpr:
    if isinstance(expr, IdentifierExpr):
        return _lower_identifier_expr(typecheck_ctx, symbol_index, expr)
    ...
    if isinstance(expr, UnaryExpr):
        return UnaryExprS(
            ...
            operand=lower_expr(typecheck_ctx, symbol_index, expr.operand),
    ...
    if isinstance(expr, BinaryExpr):
        ...
        string_concat = try_lower_string_concat_expr(
            ...
            expr,
            result_type_name,
            lower_expr=lambda nested_expr: lower_expr(typecheck_ctx, symbol_index, nested_expr),
        )
        if string_concat is not None:
            return string_concat
        return BinaryExprS(
            operator=expr.operator,
            left=lower_expr(typecheck_ctx, symbol_index, expr.left),
            right=lower_expr(typecheck_ctx, symbol_index, expr.right),
    ...
    if isinstance(expr, ArrayCtorExpr):
        return _lower_array_ctor_expr(typecheck_ctx, symbol_index, expr)

    if isinstance(expr, FieldAccessExpr):
        return _lower_field_access_expr(typecheck_ctx, symbol_index, expr)

    if isinstance(expr, IndexExpr):
        return _lower_index_expr(typecheck_ctx, symbol_index, expr)

    if isinstance(expr, CallExpr):
        return lower_call_expr(typecheck_ctx, symbol_index, expr, infer_expression_type(typecheck_ctx, expr).name)

    raise TypeError(f"Unsupported expression for semantic lowering: {type(expr).__name__}")


def lower_call_expr(
    typecheck_ctx: TypeCheckContext, symbol_index: ProgramSymbolIndex, expr: CallExpr, result_type_name: str
) -> SemanticExpr:
    ...
    args = [lower_expr(typecheck_ctx, symbol_index, arg) for arg in expr.arguments]
    ...
    if isinstance(resolved_target, ResolvedInstanceMethodCallTarget):
        return InstanceMethodCallExpr(
            ...
            receiver=lower_expr(typecheck_ctx, symbol_index, resolved_target.receiver),
    ...
    return CallableValueCallExpr(
        callee=lower_expr(typecheck_ctx, symbol_index, resolved_target.callee),
        args=args,
        type_name=result_type_name,
        span=expr.span,
    )
...
def _lower_identifier_expr(
    ...
    return lower_resolved_ref(
        resolve_identifier_ref_target(typecheck_ctx, symbol_index, expr),
...
def _lower_array_ctor_expr(
    typecheck_ctx: TypeCheckContext, symbol_index: ProgramSymbolIndex, expr: ArrayCtorExpr
) -> ArrayCtorExprS:
    ...
    return ArrayCtorExprS(
        ...
        length_expr=lower_expr(typecheck_ctx, symbol_index, expr.length_expr),
...
def _lower_index_expr(
    typecheck_ctx: TypeCheckContext, symbol_index: ProgramSymbolIndex, expr: IndexExpr
) -> IndexReadExpr:
    target_type = infer_expression_type(typecheck_ctx, expr.object_expr)
    return IndexReadExpr(
        target=lower_expr(typecheck_ctx, symbol_index, expr.object_expr),
        index=lower_expr(typecheck_ctx, symbol_index, expr.index_expr),
        type_name=infer_expression_type(typecheck_ctx, expr).name,
