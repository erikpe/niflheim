...
def resolve_for_in_element_type(ctx: TypeCheckContext, collection_type: TypeInfo, span: SourceSpan) -> TypeInfo:
    ...
    if iter_len_sig is None:
        raise TypeCheckError(f"Type '{collection_type.name}' is not iterable (missing method 'iter_len()')", span)
    require_member_visible(ctx, class_info, collection_type.name, "iter_len", "method", span)
    ...
    if iter_get_sig is None:
        raise TypeCheckError(f"Type '{collection_type.name}' is not iterable (missing method 'iter_get(i64)')", span)
    require_member_visible(ctx, class_info, collection_type.name, "iter_get", "method", span)
...
def resolve_structural_get_method_result_type(
    ...
    if method_sig is None:
        raise TypeCheckError(f"Type '{object_type.name}' is not indexable (missing method 'index_get(K)')", span)
    require_member_visible(ctx, class_info, object_type.name, "index_get", "method", span)
...
def ensure_structural_set_method_available_for_index_assignment(
    ...
    if method_sig is None:
        raise TypeCheckError(
            f"Type '{object_type.name}' is not index-assignable (missing method 'index_set(K, V)')", span
        )
    require_member_visible(ctx, class_info, object_type.name, "index_set", "method", span)
    if method_sig.is_static:
        raise TypeCheckError(
...
def resolve_structural_slice_method_result_type(
    ...
    if method_sig is None:
        raise TypeCheckError(f"Type '{object_type.name}' is not sliceable (missing method 'slice_get(i64, i64)')", span)
    require_member_visible(ctx, class_info, object_type.name, "slice_get", "method", span)
    if method_sig.is_static:
        raise TypeCheckError(
...
def resolve_structural_set_slice_method_result_type(
    ...
    if method_sig is None:
        raise TypeCheckError(
            f"Type '{object_type.name}' is not slice-assignable (missing method 'slice_set(i64, i64, U)')", span
        )
    require_member_visible(ctx, class_info, object_type.name, "slice_set", "method", span)
    if method_sig.is_static:
        raise TypeCheckError(
