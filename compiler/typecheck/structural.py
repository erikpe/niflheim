...
def resolve_for_in_element_type(
    ...
    *,
    ...
    require_member_visible(class_info, collection_type.name, "iter_len", "method", span)
    ...
    require_member_visible(class_info, collection_type.name, "iter_get", "method", span)
...
def resolve_structural_get_method_result_type(
    ...
    *,
    ...
    require_member_visible(class_info, object_type.name, "index_get", "method", span)
...
def ensure_structural_set_method_available_for_index_assignment(
    ...
    *,
    ...
    require_member_visible(class_info, object_type.name, "index_set", "method", span)
    if method_sig.is_static:
        raise TypeCheckError(
...
def ensure_index_assignment(
    ...
    *,
    ...
    if object_type.element_type is not None:
        index_type = infer_expression_type(index_expr)
...
def resolve_structural_slice_method_result_type(
    ...
    *,
    ...
    require_member_visible(class_info, object_type.name, "slice_get", "method", span)
    if method_sig.is_static:
        raise TypeCheckError(
    ...
    begin_arg_type = infer_expression_type(args[0])
    end_arg_type = infer_expression_type(args[1])
...
def resolve_structural_set_slice_method_result_type(
    ...
    *,
    ...
    require_member_visible(class_info, object_type.name, "slice_set", "method", span)
    if method_sig.is_static:
        raise TypeCheckError(
    ...
    value_arg_type = infer_expression_type(args[2])
...
def infer_array_method_call_type(
    ...
    *,
    ...
    if method_name == "index_get":
        ...
        index_type = infer_expression_type(args[0])
    ...
    if method_name == "index_set":
        ...
        index_type = infer_expression_type(args[0])
        require_array_index_type(index_type, args[0].span)
        value_type = infer_expression_type(args[1])
        require_assignable(ctx, object_type.element_type, value_type, args[1].span)
        return TypeInfo(name="unit", kind="primitive")
    if method_name == "slice_get":
        if len(args) != 2:
            raise TypeCheckError(f"Expected 2 arguments, got {len(args)}", span)
        start_type = infer_expression_type(args[0])
        end_type = infer_expression_type(args[1])
    ...
    if method_name == "slice_set":
        ...
        require_array_index_type(start_type, args[0].span)
        require_array_index_type(end_type, args[1].span)
        value_type = infer_expression_type(args[2])
