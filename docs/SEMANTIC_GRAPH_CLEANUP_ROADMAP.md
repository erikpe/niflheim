...
- Backend codegen is no longer broadly string-driven for executable expression behavior. `compiler/codegen/emitter_expr.py`, `compiler/codegen/layout.py`, `compiler/codegen/emitter_stmt.py`, and `compiler/codegen/metadata.py` now use canonical refs for the migrated cast, type-test, array, call, and temp-rooting decisions. The remaining string-heavy backend areas are declaration/signature ABI handling in `compiler/codegen/emitter_fn.py`, `compiler/codegen/layout.py`, and `compiler/codegen/generator.py`.
...
- complete
- post-step-7 reduction slices have refreshed this audit after migrating constant folding, backend cast/type-test/array handling, backend temp-rooting and ABI-shape decisions, and lowering collection dispatch helpers to canonical refs.
