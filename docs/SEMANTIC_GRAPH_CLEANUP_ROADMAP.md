...
| Field-access result type strings on nodes that already have a canonical ref | `FieldReadExpr.type_name`, `FieldLValue.type_name` | `type_ref` on the same node | canonical semantic input still used incorrectly | generic expression typing through `compiler/semantic/ir.py:expression_type_name`; backend expression emission and layout planning | These are the next small redundant-field candidates after the reference-expression slice: the canonical result type is already attached, but some consumers still reach for the copied string. |
| Executable result type strings on nodes that now also carry canonical refs | `LiteralExprS.type_name`, `UnaryExprS.type_name`, `BinaryExprS.type_name`, `FunctionCallExpr.type_name`, `StaticMethodCallExpr.type_name`, `InstanceMethodCallExpr.type_name`, `InterfaceMethodCallExpr.type_name`, `ConstructorCallExpr.type_name`, `CallableValueCallExpr.type_name`, `IndexReadExpr.type_name`, `SliceReadExpr.type_name`, `ArrayCtorExprS.type_name`, `SyntheticExpr.type_name` | `type_ref` on the same node | canonical semantic input still used incorrectly | constant folding, `expression_type_name(...)`, and much of `compiler/codegen/emitter_expr.py` | Step `1.4` added canonical result refs for these nodes, but executable consumers still branch on the copied string for integer width, return ABI shape, and runtime dispatch selection. They are removal candidates only after those consumers migrate. |
...
- Semantic optimizations still use string types directly for executable behavior. Constant folding in `compiler/semantic/optimizations/constant_folding.py` still compares `LocalRefExpr.type_name`, branches on `UnaryExprS.type_name` and `BinaryExprS.type_name`, and interprets `CastExprS.target_type_name` directly.
- Lowering-time collection and nominal dispatch helpers still consume strings for compatibility-oriented symbol lookup and runtime-kind selection in `compiler/semantic/lowering/collections.py` and `compiler/semantic/lowering/ids.py`.
- Backend codegen remains the largest string-heavy subsystem. The main readers are still `compiler/codegen/emitter_expr.py`, `compiler/codegen/emitter_stmt.py`, `compiler/codegen/layout.py`, `compiler/codegen/emitter_fn.py`, and `compiler/codegen/metadata.py`.
...
- The remaining high-value migration targets are now narrower than they were at step `1.1`: local use-site result strings, receiver and target compatibility strings, collection element/value compatibility strings, and field-access result strings still duplicate canonical information that later consumers should stop reading directly.
