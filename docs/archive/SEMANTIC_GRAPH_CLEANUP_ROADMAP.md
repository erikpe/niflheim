...
| --- | --- | --- | --- | --- | --- |
| Removed declaration and signature compatibility fields | `SemanticField.type_name`, `SemanticParam.type_name`, `SemanticInterfaceMethod.return_type_name`, `SemanticFunction.return_type_name`, `SemanticMethod.return_type_name` | `type_ref` or `return_type_ref` on the same node | removed in completed slices | no production readers remain | The semantic IR no longer stores these copied declaration/signature strings. Constructor synthesis, ABI shape decisions, and linker entrypoint validation all now use canonical refs directly. |
| Removed owner-local declared type cache | `SemanticLocalInfo.type_name` | `SemanticLocalInfo.type_ref` | removed in completed slices | no production readers remain | Owner-local type display now derives on demand from canonical refs via shared relative-display helpers, so local metadata no longer stores a copied readable type string. |
...
| Removed receiver and target compatibility strings | `FieldReadExpr.receiver_type_name`, `FieldLValue.receiver_type_name`, `InstanceMethodCallExpr.receiver_type_name`, `InterfaceMethodCallExpr.receiver_type_name`, `CastExprS.target_type_name`, `TypeTestExprS.target_type_name` | `receiver_type_ref` or `target_type_ref` on the same node | removed in completed slices | no production readers remain | Bound receiver display and cast/type-test target rendering now derive from canonical refs directly. Lowering no longer threads copied receiver or target strings through semantic nodes, and the remaining backend diagnostic path renders from canonical refs. |
| Removed collection element and value compatibility strings | `SemanticForIn.element_type_name`, `ArrayCtorExprS.element_type_name`, `IndexLValue.value_type_name`, `SliceLValue.value_type_name` | `element_type_ref` or `value_type_ref` on the same node | removed in completed slices | no production readers remain | Array constructor emission, lowered `for-in` iteration emission, and array runtime-dispatch selection all use canonical refs directly. Lowering no longer threads copied collection element or assignment value strings through semantic or lowered nodes. |
| Removed field-access result type strings on nodes that already have a canonical ref | `FieldReadExpr.type_name`, `FieldLValue.type_name` | `type_ref` on the same node | removed in completed slices | no production readers remain | Field-read display now derives through canonical `type_ref` rendering in `expression_type_name(...)`, and field lvalues no longer store redundant result-type strings. Backend expression emission and layout planning were already independent of these copied strings before removal. |
| Removed executable result type strings on nodes that now also carry canonical refs | `LiteralExprS.type_name`, `UnaryExprS.type_name`, `BinaryExprS.type_name`, `FunctionCallExpr.type_name`, `StaticMethodCallExpr.type_name`, `InstanceMethodCallExpr.type_name`, `InterfaceMethodCallExpr.type_name`, `ConstructorCallExpr.type_name`, `CallableValueCallExpr.type_name`, `IndexReadExpr.type_name`, `SliceReadExpr.type_name`, `ArrayCtorExprS.type_name`, `SyntheticExpr.type_name` | `type_ref` on the same node | removed in completed slices | no production readers remain | Generic expression display now renders from canonical `type_ref` through `expression_type_name(...)`, so these executable result caches no longer need to be stored on semantic nodes. Constant folding and backend expression emission were already using canonical refs for behavioral decisions before the cache removal. |
| Removed fixed-result compatibility display strings | `NullExprS.type_name`, `ArrayLenExpr.type_name`, `StringLiteralBytesExpr.type_name` | built-in `type_ref` on the same node | removed in completed slices | no production readers remain | These fixed-result nodes now rely on their built-in canonical refs for generic expression display, so the last copied executable display strings have been eliminated and the semantic IR tail is symmetrical with the broader executable-cache cleanup. |
...
- The remaining meaningful duplication is now lower priority than it was at step `1.1`: only the fixed-result display strings remain as low-value compatibility caches.
...
- Fixed-result default strings are the remaining acceptable temporary compatibility stores at the end of step 1 alongside the other low-priority display caches because they still support compatibility rendering paths that have not yet moved to canonical helpers.

Step 1.1 status:

- complete
- post-step-7 reduction slices have refreshed this audit after migrating constant folding, backend cast/type-test/array handling, backend temp-rooting and ABI-shape decisions, lowering collection dispatch helpers, linker entrypoint validation, and declaration/signature lowering payloads to canonical refs.
