...
def make_target_input(program: BackendProgram) -> BackendTargetInput:
...
def unit_function_backend_program(
    *,
    ...
    if param_debug_names is None:
        param_names = tuple(f"arg{index}" for index in range(len(param_type_names)))
    else:
        param_names = param_debug_names
