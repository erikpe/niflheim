...
def test_emit_source_asm_emits_branch_edges_and_single_epilogue(tmp_path) -> None:
    ...
    assert "    cmp x0, #0" in choose_body
    assert f"    b.eq {false_label}" in choose_body
    assert f"    b {true_label}" in choose_body
    assert asm.count(f"{epilogue_label(choose_label)}:") == 1
    assert asm.count(f"    b {epilogue_label(choose_label)}") >= 2
...
def test_emit_source_asm_emits_loop_backedges_and_exit_edges(tmp_path) -> None:
    ...
    assert f"    b.eq .L{loop_label}_b{exit_block.block_id.ordinal}" in asm
    assert f"    b .L{loop_label}_b{cond_block.block_id.ordinal}" in asm
