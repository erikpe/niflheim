...
def test_emit_source_asm_emits_pooled_string_literal_bytes_and_runtime_helper(tmp_path) -> None:
    ...
    assert asm.count("    adrp x0, __nif_str_lit_0") == 2
    assert asm.count("    add x0, x0, :lo12:__nif_str_lit_0") == 2
    assert asm.count("    bl rt_array_from_bytes_u8") == 2
    assert asm.count("    bl __nif_method_main__Str_from_u8_array") == 2
