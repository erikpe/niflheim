...
def test_cli_debug_logs_respect_verbosity_threshold(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    assert captured.err.count("nifc: debug: Optimization pass algebraic_simplify simplified ") == 2
    assert captured.err.count("nifc: debug: Optimization pass algebraic_simplify completed in") == 2
    ...
    assert "nifc: info: Lowering backend IR" in captured.err
    assert "nifc: debug: Backend optimization pass dead_pure_definition_elimination removed " in captured.err
    ...
    assert "nifc: debug: Backend optimization pass algebraic_simplify simplified " in captured.err
    assert "nifc: debug: Backend optimization pass algebraic_simplify completed in" in captured.err
