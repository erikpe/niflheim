...
static uint64_t g_cycle_index = 0;
static RtGcStats g_cycle_before = {0};
static uint64_t g_cycle_collect_start_ns = 0;
static uint64_t g_cycle_mark_start_ns = 0;
static uint64_t g_cycle_mark_end_ns = 0;
static uint64_t g_cycle_sweep_start_ns = 0;
static uint64_t g_cycle_sweep_end_ns = 0;
...
static void rt_gc_trace_summary_atexit(void) {
    ...
    if (g_gc_cycle_count == 0u || g_gc_timing_start_ns == 0u) {
        return;
    }
...
void rt_gc_trace_collect_begin(void) {
    g_cycle_index = g_gc_cycle_count + 1u;
    g_cycle_before = rt_gc_get_stats();

    if (!rt_gc_trace_is_enabled()) {
        g_cycle_collect_start_ns = 0u;
        g_cycle_mark_start_ns = 0u;
        g_cycle_mark_end_ns = 0u;
        g_cycle_sweep_start_ns = 0u;
        g_cycle_sweep_end_ns = 0u;
        return;
    }

    g_cycle_collect_start_ns = rt_time_now_ns_impl();
    if (g_gc_timing_start_ns == 0u) {
        g_gc_timing_start_ns = g_cycle_collect_start_ns;
    }
    ...
    fprintf(
        ...
        " threshold=%" PRIu64 "B\n",
        g_cycle_index,
        g_cycle_before.allocated_bytes,
        g_cycle_before.live_bytes,
        g_cycle_before.tracked_object_count,
        g_cycle_before.next_gc_threshold
    );
}
...
void rt_gc_trace_mark_begin(void) {
    ...
    g_cycle_mark_start_ns = rt_time_now_ns_impl();
}
...
void rt_gc_trace_mark_end(void) {
    ...
    g_cycle_mark_end_ns = rt_time_now_ns_impl();
}
...
void rt_gc_trace_sweep_begin(void) {
    ...
    g_cycle_sweep_start_ns = rt_time_now_ns_impl();
}
...
void rt_gc_trace_sweep_end(void) {
    ...
    g_cycle_sweep_end_ns = rt_time_now_ns_impl();
}


void rt_gc_trace_collect_end(void) {
    g_gc_cycle_count = g_cycle_index;
    ...
    const uint64_t collected_bytes = (g_cycle_before.allocated_bytes >= after_stats.live_bytes)
        ? (g_cycle_before.allocated_bytes - after_stats.live_bytes)
        : 0u;
    const uint64_t collected_objects = (g_cycle_before.tracked_object_count >= after_stats.tracked_object_count)
        ? (g_cycle_before.tracked_object_count - after_stats.tracked_object_count)
        : 0u;

    uint64_t mark_ns = 0u;
    uint64_t sweep_ns = 0u;
    uint64_t collect_ns = 0u;

    if (g_cycle_mark_end_ns >= g_cycle_mark_start_ns) {
        mark_ns = g_cycle_mark_end_ns - g_cycle_mark_start_ns;
    }
    if (g_cycle_sweep_end_ns >= g_cycle_sweep_start_ns) {
        sweep_ns = g_cycle_sweep_end_ns - g_cycle_sweep_start_ns;
    }
    if (g_cycle_sweep_end_ns >= g_cycle_collect_start_ns) {
        collect_ns = g_cycle_sweep_end_ns - g_cycle_collect_start_ns;
    }
...
void rt_gc_trace_reset(void) {
    g_gc_trace_enabled = -1;
    g_gc_cycle_count = 0u;
    g_gc_timing_start_ns = 0u;
    g_gc_total_collect_ns = 0u;
    g_gc_total_mark_ns = 0u;
    g_gc_total_sweep_ns = 0u;
    g_gc_summary_emitted = 0;
    g_cycle_index = 0u;
    g_cycle_before.allocated_bytes = 0u;
    g_cycle_before.live_bytes = 0u;
    g_cycle_before.next_gc_threshold = 0u;
    g_cycle_before.tracked_object_count = 0u;
    g_cycle_collect_start_ns = 0u;
    g_cycle_mark_start_ns = 0u;
    g_cycle_mark_end_ns = 0u;
    g_cycle_sweep_start_ns = 0u;
